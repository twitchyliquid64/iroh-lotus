...
| Crate  | Role |
|--------|------|
| `wire` | Types for serializing messages on the wire. CBOR2 canonical used so messages have hash stability and can be signed. The `Value` <-> iroh address conversions sit behind the `iroh` feature (on for `state` and `lotusd`, off for `lotusd-rpc`), so a control-socket client builds without a QUIC stack. |
...
| `lotusd-rpc` | The protocol lotusd speaks on its local socket. Length-prefixed canonical CBOR frames; one connection carries one request and the stream of responses to it. A `Method` pairs a request with the response type that answers it, so callers never name the response, and says whether it is answered once (`AnsweredOnce`, all `call` accepts) or streams (read through `Call`, itself a `Stream`). `GetStatus` answers with everything `lotusctl status` prints — identity, endpoint, chain range, kept peers, inbound count — carrying iroh ids as strings so clients need not depend on iroh. |
| `lotus-sdk` | A running lotusd as a Rust API, for programs other than `lotusctl`. A `Client` names the control socket (`Client::discover` finds it where `lotusd` keeps it, falling back to the machine-wide `/var/lib/lotus` when the user's own state directory does not exist — clients only; `lotusd` never falls back) and opens one connection per request; every protocol method has a typed method on it, `call`/`stream` take any `Method`, and `Streaming` is the answers to a streaming request as a `tokio_stream::Stream`. Re-exports the request, response and `wire` value types a program touches, so it depends on this crate alone. |
| `lotusweb` | A web view onto a running lotusd, over `lotus-sdk` and axum: a sidebar of namespaces beside a main pane walked like a file browser, with breadcrumbs, JSON editors for leaves, add/append/replace/delete forms for containers. Plain HTML over htmx 4 (vendored in `static/`): `hx-target` is inherited from `body` (htmx 4 inherits nothing implicitly), every link and form fetches the main pane alone and the sidebar rides along out of band; a plain navigation or history restore gets the whole document. A location's URL, `/ns/<namespace>/<path>`, spells the path exactly as `lotusctl` does (`servers[0].host`), and the method says what happens there: `GET` shows, `PUT` replaces, `POST` adds an entry, `PATCH` amends in place (adds to an integer, or renames a map entry: a set at the new key then a delete at the old, the chain having no move), `DELETE` removes. Values are plain JSON, as `lotusctl` speaks them, so a trusted key can be seen but not written from the page. `/chain` is `lotusctl chain` as a page: one stanza per envelope, newest first, its targets linked into the browser, bounded by `?limit=` and `?since=` as that command is by its flags; reached from the bottom of the sidebar, apart from the namespaces. |
