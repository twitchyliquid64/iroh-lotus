...
| `lotusweb` | A web view onto a running lotusd, over `lotus-sdk` and axum: a sidebar of namespaces beside a main pane walked like a file browser, with breadcrumbs, JSON editors for leaves, add/append/replace/delete forms for containers. Plain HTML over htmx 4 (vendored in `static/`): `hx-target` is inherited from `body` (htmx 4 inherits nothing implicitly), every link and form fetches the main pane alone and the sidebar rides along out of band; a plain navigation or history restore gets the whole document. A location's URL, `/ns/<namespace>/<path>`, spells the path exactly as `lotusctl` does (`servers[0].host`), and the method says what happens there: `GET` shows, `PUT` replaces, `POST` adds an entry, `PATCH` adds to an integer, `DELETE` removes. Values are plain JSON, as `lotusctl` speaks them, so a trusted key can be seen but not written from the page. |
